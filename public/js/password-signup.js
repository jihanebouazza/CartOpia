...
function validatePassword(password) {
  ...
  passwordDivs[2]
    .querySelector("i")
    .classList.toggle("fa-check", hasSpecialChars);
...
document
  .querySelector(".password-name")
  .addEventListener("input", function (e) {
    validatePassword(e.target.value);
  });

document.querySelector(".password-name").addEventListener("focus", function () {
  document.querySelector(".password-validation").style.display = "flex";
});

document.querySelector(".password-name").addEventListener("blur", function () {
